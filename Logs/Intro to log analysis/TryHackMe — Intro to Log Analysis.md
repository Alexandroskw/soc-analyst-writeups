...
> [!NOTA] User Behavior Analytics (UBA)
...
> [!INFO]- Listas de payloads para las firmas comunes de ataque
...
![[Uso del comando cat (bat en este caso).png]]
...
![[Pasted image 20260821215844.png]]
#### `tail`
...
tail apache.log
```
![[Pasted image 20260821231042.png]]
```bash
...
...
```
Con la bandera `-n` se ordena la salida de forma numérica y si se agrega la bandera `-r` lo hace de forma descendentEn esta caso COn lon la bandera `-n` se hace ordena la salida con dde num´érica, es decir primer, en de forma num´érica y si se agrega la rla bandera `-r` lo hace ade forma ascendente.
> [!NOTA]
> Para entender mejor estos comandos, se recomienda leer [[TryHackMe — Intro to logs]]
...
**Respuesta: 52**
**Precaución**: No hacer caso a la pista que pone la sala. Es mejor utilizar `grep`:
...
`awk '{print $1}' apache.log | sort | uniq -c | sort -nr | head -1`
...
![[Subir archivo en CyberChef.png]]
